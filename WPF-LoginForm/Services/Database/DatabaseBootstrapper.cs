...
namespace WPF_LoginForm.Services.Database
{
    public static class DatabaseBootstrapper
    {
        ...
        public static bool Run()
        {
            ...
            {
                // NEW: Even if ready, check for missing columns (Migration)
            ...
            if (DbConnectionFactory.CurrentDatabaseType == DatabaseType.SqlServer && sqlStatus == DbStatus.Ready) { PerformMigrations(DatabaseType.SqlServer); return true; }
            if (DbConnectionFactory.CurrentDatabaseType == DatabaseType.PostgreSql && pgStatus == DbStatus.Ready) { PerformMigrations(DatabaseType.PostgreSql); return true; }
        ...
        // --- NEW: Migration Logic (Adds Role column if missing) ---
        ...
        private static bool AskToInitialize(string providerName, bool needsDbCreation)
        {
            ...
            var result = MessageBox.Show($"{providerName} connection detected.\nStatus: {task} required.\n\nDo you want to proceed?", "Database Initialization", MessageBoxButton.YesNo, MessageBoxImage.Warning);
        ...
        private static bool InitializeAndSet(DatabaseType type, bool createDb)
        {
            try
            {
                ...
                if (createDb) { CreateDatabase(type, "LoginDb"); CreateDatabase(type, "MainDataDb"); }
                ...
                MessageBox.Show("Database initialized successfully! Default user: admin / admin", "Success", MessageBoxButton.OK, MessageBoxImage.Information);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private static DbStatus CheckStatus(DatabaseType type, out string errorMsg)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = (type == DatabaseType.SqlServer) ? "SELECT COUNT(*) FROM sys.tables WHERE name = 'User'" : "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'User'";
            ...
            catch (Exception ex)
            {
                ...
                if (!isMissingDb) // Double check via System DB
                {
                    try
                    {
                        string sysDb = (type == DatabaseType.SqlServer) ? "master" : "postgres";
                        if (type == DatabaseType.SqlServer) { var b = new SqlConnectionStringBuilder(connStr); b.InitialCatalog = sysDb; using (var c = new SqlConnection(b.ConnectionString)) { c.Open(); } }
                        else { var b = new NpgsqlConnectionStringBuilder(connStr); b.Database = sysDb; using (var c = new NpgsqlConnection(b.ConnectionString)) { c.Open(); } }
                        isMissingDb = true;
                    }
                    catch { isMissingDb = false; }
                }

                if (isMissingDb) { errorMsg = "Database 'LoginDb' does not exist."; return DbStatus.MissingDb; }
        ...
        private static void InitializeAuthSqlServer(IDbConnection conn)
        {
            ...
            {
                string createUser = @"CREATE TABLE [User] ([Id] INT IDENTITY(1,1) PRIMARY KEY, [Username] NVARCHAR(50) NOT NULL UNIQUE, [Password] NVARCHAR(100) NOT NULL, [Name] NVARCHAR(50), [LastName] NVARCHAR(50), [Email] NVARCHAR(100), [Role] NVARCHAR(50) DEFAULT 'User')";
        ...
        private static void InitializeDataSqlServer(IDbConnection conn)
        {
            ...
            {
                string createMap = @"CREATE TABLE [ColumnHierarchyMap] ([Id] INT IDENTITY(1,1) PRIMARY KEY, [OwningDataTableName] NVARCHAR(100), [Part1Value] NVARCHAR(200), [Part2Value] NVARCHAR(200), [Part3Value] NVARCHAR(200), [Part4Value] NVARCHAR(200), [CoreItemDisplayName] NVARCHAR(200), [ActualDataTableColumnName] NVARCHAR(200))";
        ...
        private static void InitializeAuthPostgres(IDbConnection conn)
        {
            ...
            {
                string createUser = @"CREATE TABLE ""User"" (""Id"" SERIAL PRIMARY KEY, ""Username"" VARCHAR(50) NOT NULL UNIQUE, ""Password"" VARCHAR(100) NOT NULL, ""Name"" VARCHAR(50), ""LastName"" VARCHAR(50), ""Email"" VARCHAR(100), ""Role"" VARCHAR(50) DEFAULT 'User')";
                ExecuteNonQuery(conn, createUser);
                ExecuteNonQuery(conn, "INSERT INTO \"User\" (\"Username\", \"Password\", \"Name\", \"LastName\", \"Email\", \"Role\") VALUES ('admin', 'admin', 'System', 'Admin', 'admin@biosun.com', 'Admin')");
            }
        ...
        private static void InitializeDataPostgres(IDbConnection conn)
        {
            ...
            {
                string createMap = @"CREATE TABLE ""ColumnHierarchyMap"" (""Id"" SERIAL PRIMARY KEY, ""OwningDataTableName"" VARCHAR(100), ""Part1Value"" VARCHAR(200), ""Part2Value"" VARCHAR(200), ""Part3Value"" VARCHAR(200), ""Part4Value"" VARCHAR(200), ""CoreItemDisplayName"" VARCHAR(200), ""ActualDataTableColumnName"" VARCHAR(200))";
                ExecuteNonQuery(conn, createMap);
            }
        }
