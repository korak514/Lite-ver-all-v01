...
namespace WPF_LoginForm.ViewModels
{
    public class HomeViewModel : ViewModelBase
    {
        ...
        public HomeViewModel(IDataRepository dataRepository, IDialogService dialogService, ILogger logger)
        {
            ...
            RecolorChartsCommand = new ViewModelCommand(p => LoadAllChartsData()); // Randomizes colors if no map provided
        ...
        private async void LoadAllChartsData(Dictionary<(int, string), string> colorMap = null)
        {
            ...
            {
                for (int i = 1; i <= 5; i++)
                    _dashboardConfigurations.Add(new DashboardConfiguration { ChartPosition = i, IsEnabled = false });
            }
            ...
            {
                _logger?.LogError($"Error loading charts: {ex.Message}");
            }
        }
        ...
        private async Task ProcessChartConfigurationAsync(DashboardConfiguration config, Dictionary<(int, string), string> colorMap, CancellationToken token)
        {
            ...
            try
            {
                ...
                // Process Data off the UI thread using the Chart Service
                var chartResult = await Task.Run(() => _chartService.ProcessChartData(
                    dt, config, IsFilterByDate, IgnoreNonDateData,
                    StartMonthSliderValue, EndMonthSliderValue, SliderMaximum, colorMap), token);
            ...
            {
                _logger?.LogError($"Error processing chart position {config.ChartPosition}", ex);
            }
        }
        ...
        private void ApplyChartResultToUI(int position, DashboardChartService.ChartResultDto result)
        {
            ...
            foreach (var s in result.Series)
            {
                ...
                if (s.SeriesType == "Pie")
                {
                    newSeries.Add(new PieSeries
                    {
                        ...
                        DataLabels = true,
                        LabelPoint = p => string.Format("{0:P0}", p.Participation),
                        Fill = colorBrush
                    });
                }
                else if (s.SeriesType == "Line")
                {
                    if (result.IsDateAxis)
                    {
                        ...
                        cv.AddRange(s.Values.Cast<DateTimePoint>());
                        newSeries.Add(new LineSeries { Title = s.Title, Values = cv, PointGeometry = DefaultGeometries.None, Stroke = colorBrush });
                    }
                    else
                    {
                        var cv = new ChartValues<double>();
                        cv.AddRange(s.Values.Cast<double>());
                        newSeries.Add(new LineSeries { Title = s.Title, Values = cv, PointGeometry = DefaultGeometries.Circle, Stroke = colorBrush });
                    }
                }
                else // Bar
                {
            ...
            {
                targetY.Add(new Axis { Title = "Values", MinValue = 0, Foreground = axisColor });

                if (result.IsDateAxis)
                {
                    targetX.Add(new Axis { LabelFormatter = DateFormatter, Separator = new Separator { IsEnabled = false }, Foreground = axisColor });
                }
                else
                {
                    targetX.Add(new Axis { Labels = result.XAxisLabels, Separator = new Separator { IsEnabled = false }, Foreground = axisColor });
                }
            }
        }
        ...
        {
            Chart1X.Clear(); Chart1Y.Clear();
            Chart2X.Clear(); Chart2Y.Clear();
            Chart3X.Clear(); Chart3Y.Clear();
            Chart5X.Clear(); Chart5Y.Clear();
            // Don't add hidden ones unless necessary to prevent LiveCharts crashes,
            // the UI will dynamically pick up added axes when series are bound.
        }
        ...
        private async void ShowConfigurationWindow()
        {
            ...
            {
                _dashboardConfigurations = vm.GetFinalConfigurations();
                Activate();
                AutoSave();
            }
        }
        ...
        private void InitializeSliders()
        {
            if (!IsFilterByDate)
            {
                SliderMaximum = 100; StartMonthSliderValue = 0; EndMonthSliderValue = 100;
                UpdateTooltips();
                return;
            }
            if (_minSliderDate == default) return;

            // Apply strict bounds to the start and end dates
        ...
        private void UpdateDatesFromSliders()
        {
            ...
            {
                double tmp = StartMonthSliderValue;
                StartMonthSliderValue = EndMonthSliderValue;
                EndMonthSliderValue = tmp;
            }
        ...
        {
            if (IsFilterByDate && _minSliderDate != default)
            {
                StartSliderTooltip = _minSliderDate.AddMonths((int)_startMonthSliderValue).ToString("MMM yyyy");
                EndSliderTooltip = _minSliderDate.AddMonths((int)_endMonthSliderValue).ToString("MMM yyyy");
            }
            else
            {
                StartSliderTooltip = $"{StartMonthSliderValue:F0}%";
                EndSliderTooltip = $"{EndMonthSliderValue:F0}%";
            }
        }
        ...
        private void LoadSelectedDashboardFile(string f)
        {
            try
            {
                LoadSnapshotFromFile(Path.Combine(Settings.Default.ImportIsRelative ? AppDomain.CurrentDomain.BaseDirectory : Settings.Default.ImportAbsolutePath, f));
            }
            catch { }
        }
        ...
        private void AutoSave()
        {
            var snap = new DashboardSnapshot
            {
                ...
                SeriesData = new List<ChartSeriesSnapshot>() // Don't save the actual massive dataset points, just config
            };
        ...
        private void TryLoadAutoSave()
        {
            try
            {
                if (Settings.Default.AutoImportEnabled)
                {
                    ...
                    string def = Settings.Default.ImportFileName;
                    string path = Settings.Default.ImportIsRelative ? AppDomain.CurrentDomain.BaseDirectory : Settings.Default.ImportAbsolutePath;
                    if (!string.IsNullOrEmpty(def) && File.Exists(Path.Combine(path, def)))
                    {
                        LoadSnapshotFromFile(Path.Combine(path, def));
                        if (DashboardFiles.Contains(def))
                        {
                            _selectedDashboardFile = def;
                            OnPropertyChanged(nameof(SelectedDashboardFile));
                        }
                        return;
                    }
                }
                var s = _storageService.LoadSnapshot();
                if (s != null) LoadDashboardFromSnapshot(s);
            }
            catch { }
        }
        ...
        private void ImportConfiguration()
        {
            ...
            if (d.ShowDialog() == true) LoadSnapshotFromFile(d.FileName);
        }
        ...
        private void ExportConfiguration()
        {
            ...
            // If Snapshot export was true, we could theoretically map points into SeriesData,
            // but for safety/memory, we usually just export config.
        ...
        private void LoadDashboardFromSnapshot(DashboardSnapshot s)
        {
            ...
            try
            {
                ...
                if (s.InitialDateForConversion != default)
                    InitialDateForConversion = s.InitialDateForConversion;

                // Fix: Restore Dates properly without crashing or hitting MinValue
                if (s.StartDate != default && s.EndDate != default)
                {
            ...
            {
                if (wasActive && !_isActive) _isActive = true;
            }
        }
        ...
        private void ExecuteChartClick(object parameter)
        {
            ...
            {
                int pos = 0;
                if (Chart1Series.Contains(series)) pos = 1;
                else if (Chart2Series.Contains(series)) pos = 2;
                else if (Chart3Series.Contains(series)) pos = 3;
                else if (Chart4Series.Contains(series)) pos = 4;
                else if (Chart5Series.Contains(series)) pos = 5;
