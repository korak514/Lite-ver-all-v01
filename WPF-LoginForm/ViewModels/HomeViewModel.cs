// In WPF_LoginForm.ViewModels/HomeViewModel.cs
using System;
...
namespace WPF_LoginForm.ViewModels
{
    public class HomeViewModel : ViewModelBase
    {
        ...
        public DateTime StartDate { get => _startDate; set { if (_startDate == value) return; _startDate = value; OnPropertyChanged(); if (!_isUpdatingDates) { LoadAllChartsData(); UpdateSlidersFromDates(); } } }

        private DateTime _endDate;
        public DateTime EndDate { get => _endDate; set { if (_endDate == value) return; _endDate = value; OnPropertyChanged(); if (!_isUpdatingDates) { LoadAllChartsData(); UpdateSlidersFromDates(); } } }
        ...
        public double StartMonthSliderValue { get => _startMonthSliderValue; set { if (SetProperty(ref _startMonthSliderValue, value) && !_isUpdatingDates) { UpdateDatesFromSliders(); } } }

        private double _endMonthSliderValue;
        public double EndMonthSliderValue { get => _endMonthSliderValue; set { if (SetProperty(ref _endMonthSliderValue, value) && !_isUpdatingDates) { UpdateDatesFromSliders(); } } }
        ...
        public HomeViewModel(IDataRepository dataRepository, IDialogService dialogService)
        {
            ...
            DateFormatter = value => (value >= DateTime.MinValue.Ticks) ? new DateTime((long)value).ToString("d") : string.Empty;
        }
        ...
        {
            Chart1Series.Clear(); Chart1X.Clear(); Chart1Y.Clear();
            Chart2Series.Clear(); Chart2X.Clear(); Chart2Y.Clear();
            Chart3Series.Clear(); Chart3X.Clear(); Chart3Y.Clear();
            Chart4Series.Clear();
            Chart5Series.Clear(); Chart5X.Clear(); Chart5Y.Clear();
        }
        ...
        private void UpdateChartData(DataTable dataTable, DashboardConfiguration config, Dictionary<(int, string), string> colorMap = null)
        {
            ...
            // --- NEW DYNAMIC PARSING LOGIC ---
            Func<object, double> safeConvertToDouble = (obj) =>
            {
                if (obj == null || obj == DBNull.Value) return 0.0;

                // Choose the culture based on the user's setting for this specific chart
            ...
            switch (config.ChartPosition) { case 1: Chart1Series.Clear(); Chart1Series.AddRange(newSeries); Chart1X.Clear(); if (newXAxis != null) Chart1X.AddRange(newXAxis); Chart1Y.Clear(); if (newYAxis != null) Chart1Y.AddRange(newYAxis); break; case 2: Chart2Series.Clear(); Chart2Series.AddRange(newSeries); Chart2X.Clear(); if (newXAxis != null) Chart2X.AddRange(newXAxis); Chart2Y.Clear(); if (newYAxis != null) Chart2Y.AddRange(newYAxis); break; case 3: Chart3Series.Clear(); Chart3Series.AddRange(newSeries); Chart3X.Clear(); if (newXAxis != null) Chart3X.AddRange(newXAxis); Chart3Y.Clear(); if (newYAxis != null) Chart3Y.AddRange(newYAxis); break; case 4: Chart4Series.Clear(); Chart4Series.AddRange(newSeries); break; case 5: Chart5Series.Clear(); Chart5Series.AddRange(newSeries); Chart5X.Clear(); if (newXAxis != null) Chart5X.AddRange(newXAxis); Chart5Y.Clear(); if (newYAxis != null) Chart5Y.AddRange(newYAxis); break; }
        }
        ...
        private void ProcessDailyDateChart(DataTable dataTable, DashboardConfiguration config, SeriesCollection newSeries, AxesCollection newXAxis, Func<string, Brush> brushProvider, Func<object, double> numberParser)
        {
            ...
            if (config.AggregationType != "Daily")
            {
                ...
                foreach (var seriesConfig in config.Series.Where(s => !string.IsNullOrEmpty(s.ColumnName) && dataTable.Columns.Contains(s.ColumnName)))
                {
                    ...
                    {
                        newSeries.Add(new LineSeries { Title = seriesConfig.ColumnName, Values = values, PointGeometry = DefaultGeometries.None, Stroke = brushProvider(seriesConfig.ColumnName) });
                    }
                    else
                    {
            ...
            {
                var simplifiedLabels = GetSimplifiedDateLabels(allDataPoints.Select(p => p.Date));
                foreach (var seriesConfig in config.Series.Where(s => !string.IsNullOrEmpty(s.ColumnName) && dataTable.Columns.Contains(s.ColumnName)))
                {
                    if (config.ChartType == "Line")
                    {
        ...
        private string[] GetSimplifiedDateLabels(IEnumerable<DateTime> dates) { if (dates == null || !dates.Any()) return null; var dateList = dates.ToList(); var firstDate = dateList.First().ToString("d"); if (dateList.Count < 3) { var lastDate = dateList.Last().ToString("d"); return firstDate == lastDate ? new[] { firstDate } : new[] { firstDate, lastDate }; } else { var middleDate = dateList[dateList.Count / 2].ToString("d"); var lastDate = dateList.Last().ToString("d"); return new[] { firstDate, middleDate, lastDate }; } }
